...
class BrushstrokeOptimizer:
    ...
    def _initialize(self):
        ...
        self.init_loss=[None]
        if isinstance(self.canvas_color,str) is False:
            baseimage = self.canvas_color
        else:
            baseimage = 0*self.content_img_np
        canvas_color_features = self.vgg.extract_features(ops.preprocess_img(tf.constant(name="basecanvas",value=baseimage[::2,::2,:], dtype=self.dtype)))
        ...
        self.z_order = tf.Variable(name="z_order",initial_value=np.random.normal(0,self.zstep,location.shape[0]),dtype=self.dtype)
        ...
        if self.zsorting:
            self.varlist.append(self.z_order)
    ...
    def render(self):
        return self._render()
    def _render(self):
        curve_points,locations,colors,widths,z_orders = ops.sample_quadratic_bezier_curve2(s=self.curve_s + self.location, #
                                                         e=self.curve_e + self.location,
                                                         c=self.curve_c + self.location,
                                                         colors = self.color,
                                                         widths = self.width,
                                                         z_orders = self.z_order,
                                                         num_points=self.S,
                                                         dtype=self.dtype)
...
#                                   weights=[1, 1, 1, 1, 1])*self.style_weight
    ...
    def _constraints(self):
        ...
        loc_sclip = tf.concat([tf.clip_by_value(loc_sx, 0, self.canvas_height)-loc_x, tf.clip_by_value(loc_sy, 0, self.canvas_width)-loc_y], axis=-1)
        ...
        loc_eclip = tf.concat([tf.clip_by_value(loc_ex, 0, self.canvas_height)-loc_x, tf.clip_by_value(loc_ey, 0, self.canvas_width)-loc_y], axis=-1)
        ...
        loc_cclip = tf.concat([tf.clip_by_value(loc_cx, 0, self.canvas_height)-loc_x, tf.clip_by_value(loc_cy, 0, self.canvas_width)-loc_y], axis=-1)
        self.curve_c.assign(loc_cclip)
